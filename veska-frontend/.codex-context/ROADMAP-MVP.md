...
Todo recurso debe estar asociado a una empresa mediante `tenant_id`.
...
Un usuario solo puede acceder a filas donde el `tenant_id` coincide con su empresa activa.
...
* todas las tablas críticas tienen `tenant_id`;
...
Solo usuarios con rol `platform_admin` deben acceder.
