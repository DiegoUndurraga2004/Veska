...
Todo dato perteneciente a una empresa/cliente debe estar asociado mediante
`tenant_id` o ser alcanzable desde una relación tenant-scoped.  
`users` representa identidades globales de Veska y no tiene `tenant_id`; el
acceso de una identidad a una empresa se valida mediante `tenant_memberships`.
Esto aplica a membresías, documentos, espacios, grupos, chats, chunks,
embeddings, permisos, configuración IA y logs.
